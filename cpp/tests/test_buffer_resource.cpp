...
TEST_F(BufferSpillStatistics, RecordedWhenTheCopiesBypassMove) {
    // `move()` is not the only route: a caller can `buffer_copy` between buffers it
    // allocated itself, which both opens and closes the record.
    ...
    device_buffer = allocate(MemoryType::DEVICE);
    buffer_copy(stats, *device_buffer, *host_buffer, size);
    EXPECT_EQ(samples(), 1u);
}
...
TEST_F(BufferSpillStatistics, RecordIsClosedOnlyOnce) {
    // Two buffers can share a record, so closing it must not be repeatable.
    ...
    buffer_copy(stats, *second_host_buffer, *host_buffer, medium);  // shares the record

    device_buffer = allocate(MemoryType::DEVICE, medium);
    buffer_copy(stats, *device_buffer, *host_buffer, medium);
